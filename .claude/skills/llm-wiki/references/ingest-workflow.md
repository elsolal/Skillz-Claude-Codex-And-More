...
```
qmd update    # rebuild FTS/BM25 index for new + modified files
qmd embed     # generate vector embeddings for new chunks
```

**Why this is required:** without these two commands the new pages are not findable via `qmd query`, `qmd vsearch`, `qmd search`, or the QMD MCP server. The wiki itself has the content, but every retrieval surface that depends on the local index lags until the next manual sync.
