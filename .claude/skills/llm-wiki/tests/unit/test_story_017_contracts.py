...
class RepositoryContractsSelectionTests(unittest.TestCase):
    ...
    def test_immutable_denylist_and_symlink_boundary_override_broad_glob(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (root / "secrets").mkdir()
            (root / "secrets" / "config.yaml").write_text("token: private\n")
            (root / "credentials").mkdir()
            (root / "credentials" / "schema.json").write_text("{}\n")
            (root / "dist").mkdir()
        ...
        self.assertEqual(rejected["secrets"], "immutable_denylist")
        self.assertEqual(rejected["credentials"], "immutable_denylist")
