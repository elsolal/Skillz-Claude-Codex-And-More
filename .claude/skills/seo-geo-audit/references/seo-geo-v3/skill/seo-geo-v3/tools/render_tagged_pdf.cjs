...
console.error(
  ...
  + "  templates/Charte_PDF_SEO_GEO_V3.md et n'est PAS livrable à un client.\n"
...
const defaultThemeCandidates = [
  ...
  path.resolve(__dirname, "../skill/seo-geo-v3/assets/default_theme.json"),
];
...
const agencyName = label("identity.agency_name", theme.identity?.agency_name || "SEO/GEO Squad");
...
const title = titleMatch ? titleMatch[1].trim() : "SEO/GEO V3";
...
(async () => {
  ...
  const configuredBrowser = process.env.SEO_GEO_CHROME_PATH;
  if (configuredBrowser && !fs.existsSync(configuredBrowser)) {
    throw new Error(`SEO_GEO_CHROME_PATH pointe vers un fichier absent: ${configuredBrowser}`);
  }
