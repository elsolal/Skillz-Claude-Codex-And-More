...
_THEME_CANDIDATES = (
    ...
    _TOOL_ROOT / "skill" / "seo-geo-v3" / "assets" / "default_theme.json",
)
...
def register_fonts(body_family: str = "Arial", heading_family: str = "Arial") -> tuple[str, str]:
    ...
    if (body_family.lower() == "arial" or heading_family.lower() == "arial") and regular.is_file() and bold.is_file():
        pdfmetrics.registerFont(TTFont("SeoGeoSans", str(regular)))
        pdfmetrics.registerFont(TTFont("SeoGeoSans-Bold", str(bold)))

    def resolve(family: str, bold_face: bool) -> str:
        normalized = family.strip().lower()
        if normalized == "arial" and regular.is_file() and bold.is_file():
            return "SeoGeoSans-Bold" if bold_face else "SeoGeoSans"
...
class SeoGeoDocTemplate(BaseDocTemplate):
...
def render(input_path: Path, output_path: Path, author: str | None, theme_path: Path | None = None) -> None:
    ...
    doc.addPageTemplates([PageTemplate(id="seo-geo", frames=[frame], onPage=page_decor)])
    doc.build(story)
...
def main() -> int:
    ...
    print(
        ...
        "  son rendu NE SUIT PAS templates/Charte_PDF_SEO_GEO_V3.md, tombe en repli de police système\n"
