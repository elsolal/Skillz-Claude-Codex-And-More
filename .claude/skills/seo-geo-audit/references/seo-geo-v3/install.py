#!/usr/bin/env python3
"""Vérifie et installe le Squad SEO/GEO V3 complet dans un dossier choisi."""
...
SKILL_NAME = "seo-geo-squad-v3"
METHODOLOGY = ROOT / "skill" / "seo-geo-v3"
REQUIRED = [
    ROOT / "VERSION.json",
    ...
    ROOT / "templates" / "Charte_PDF_SEO_GEO_V3.md",
...
def check_package() -> list[str]:
    ...
    if manifest_path.is_file():
        try:
            ...
            if expected.get("product") != "SEO/GEO Squad V3":
...
def pdf_engine_notice() -> str | None:
    ...
    return (
        ...
        "  Moteur balisé (recommandé) : cd skill/seo-geo-v3 && npm ci && npx playwright install chromium\n"
        "  Moteur de secours isolé : python3 -m venv .venv-pdf && "
        ".venv-pdf/bin/python -m pip install -r skill/seo-geo-v3/requirements-pdf.txt\n"
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("Préflight réussi : Squad SEO/GEO V3 cohérent.")
