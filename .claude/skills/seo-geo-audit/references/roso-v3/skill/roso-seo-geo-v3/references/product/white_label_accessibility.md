...
document:
  ...
  methodology_name: "Propulsé par RosoAI SEO/GEO V3"
...
- conserver un identifiant de méthode/version même lorsque la marque RosoAI est masquée commercialement ;
...
Le package utilise `skill/roso-seo-geo-v3/assets/default_theme.json` comme base. Un thème JSON partiel passé avec `--theme` est fusionné avec cette base, puis validé avant tout rendu :
...
Le livrable client est composé en HTML par l'Agent 11 selon `templates/Charte_PDF_RosoAI_V3.md`, puis imprimé par ce moteur, qui n'ajoute aucun style et refuse tout document non conforme. Les polices Inter et JetBrains Mono sont embarquées dans `assets/fonts/` sous licence OFL : aucun import distant, aucune police système supposée.
