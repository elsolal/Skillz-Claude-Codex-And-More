...
Ces deux PDF suivent la charte `templates/Charte_PDF_RosoAI_V3.md`. Tu DOIS la lire en entier avant de produire quoi que ce soit, et tu DOIS respecter sa structure et sa palette à 100 %.

## Gold standard

Le dossier `exemple_livrable/` contient la paire de PDF de référence :

- `REFERENCE_Audit_Note_Strategique.pdf`
- `REFERENCE_Plan_Implementation.pdf`

**C'est le niveau de qualité, de structure et de pédagogie attendu.** La charte te dit quoi produire ; cet exemple te montre à quoi ça ressemble quand c'est bien fait.

L'utilisateur peut à tout moment te renvoyer dessus : « **Régénère au niveau de l'exemple du dossier `exemple_livrable/`.** » Dans ce cas, tu reprends ta composition et tu la remontes au niveau de l'exemple, sans discuter.

### ⚠️ Référence de FORME uniquement

Cet exemple est généré sur un site **entièrement fictif** (Atelier Brindille, fleuriste à Lyon).

Tu t'en inspires pour la **structure, le ton, la pédagogie et la densité**. Tu ne réutilises **jamais** ses données, ses chiffres, ses constats, ses concurrents ni ses formulations : ils appartiennent à une autre entreprise et à un autre audit. Toute donnée de ton document vient des registres du projet en cours.

### Sa partie « scores » est périmée

L'exemple date de la V2. Sa page de scores affiche un **score global sur 100** et **neuf dimensions** : les deux sont **interdits en V3**. Tu appliques la section 4 du bloc d'adaptation de la charte — cinq dimensions F/V/O/E/M, couverture et confiance sous chaque barre, aucune note globale. Tout le reste de l'exemple reste la référence.

### Ce que « au niveau de l'exemple » veut dire concrètement
...
`templates/Charte_PDF_RosoAI_V3.md` est ta **source unique de vérité** pour la mise en page.
...
- la charte `templates/Charte_PDF_RosoAI_V3.md` a été lue en entier ;
...
1. `templates/Charte_PDF_RosoAI_V3.md` — bloc d'adaptation puis corps ;
...
1. Tu rends chaque HTML en PDF avec `skill/roso-seo-geo-v3/tools/render_html_pdf.cjs`. Ce moteur imprime sans toucher à ta mise en page.
