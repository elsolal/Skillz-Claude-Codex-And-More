...
Preserve each agent prompt as authoritative. Execute the 11-agent order from the master orchestrator, adapt Claude-in-Chrome instructions to the current runtime capabilities, classify every factual claim as Confirmé, Déduit, or Non vérifié, double-check negative claims, and do not modify the audited site or project code. Create `audit-livrables/<client>/` deliverables only when the user asked for files or the context clearly allows audit output files; otherwise output the deliverables inline.
